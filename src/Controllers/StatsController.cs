...
public class StatsController : Controller
{
    ...
    public StatsController(ILogger<StatsController> logger, VisitTracker visitTracker)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var monthlyActivity = Cache
                .Models.Where(x => x.Date.HasValue)
    ...
    private List<MonthlyStats> NormalizeDateRange(List<MonthlyStats> stats)
    {
        ...
        for (var date = minDate; date <= maxDate; date = date.AddMonths(1))
        {
            ...
            normalizedStats.Add(new MonthlyStats { Date = date, Count = stat?.Count ?? 0 });
        }
